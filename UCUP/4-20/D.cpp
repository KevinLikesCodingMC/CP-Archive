...
void solve() {
	...
	FOR(i, 1, n) {
		dp[i][0] = dp[i][1] = 0;
		if(mx[1][i - 1] < a[i]) dp[i][1] ++;
		FOR(j, 1, i - 2) {
			{
				bool ok = 0;
				if(i & 1) ok |= a[j] > mx[j + 1][i - 1];
				else ok |= a[j] > mx[j + 1][i - 2] && a[i] > a[i - 1];
				if(j & 1) ok |= a[i] > mx[j + 2][i - 1] && a[j] > a[j + 1];
				else ok |= a[i] > mx[j + 1][i - 1];
				if(ok) dp[i][1] += dp[j][0];
			} {
				bool ok = 0;
				if(! (i & 1)) ok |= a[j] > mx[j + 1][i - 1];
